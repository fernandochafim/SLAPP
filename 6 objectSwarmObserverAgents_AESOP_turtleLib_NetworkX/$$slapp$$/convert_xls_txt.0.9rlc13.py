          ...
          if  current=='macro':
              ...
              if r>0:
                  if sh.cell_value(r-1,0)=="#": #New
                    print >>f, "#",int(sh.cell_value(r-1,1))
...
if "schedule.xls" in files:
 ...
 # sorting
 ll.sort(cmp=lambda x,y: cmp(x[0], y[0])) # so operating only on the first element
